...
export const Dropdown = ({ colorToken = 'moonpurple' }: SwitchColorProps) => {
  let bgColor = `bg-${colorToken}`;
  console.log('Dropdown :', bgColor);
  if (colorToken === 'bg-moonred') {
    return <div> {constructTemplate(bgColor)}</div>;
  } else {
    return <div> {constructTemplate(bgColor = 'bg-moonpink')}</div>;
  }
};

function constructTemplate(bgColor: string) {
  <div className="w-56 text-right top-16">
    <Menu as="div" className="relative inline-block text-left">
      <div>
        <Menu.Button
          className={`inline-flex justify-center w-full px-4 py-2 text-sm font-medium text-white rounded-md ${bgColor} hover:bg-opacity-70 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}
        >
          Options
          <ChevronDownIcon
